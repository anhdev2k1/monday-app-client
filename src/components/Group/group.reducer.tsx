...
export const updateAllGroups = createAsyncThunk(
  'update-groups-slice',
  async ({ idBoard, groups }: { idBoard: string; groups: IGroup[] }) => {
    const requestUrl = `${apiUrl}v1/api/board/${idBoard}/allgroups`;
    const transformedGroups = groups.map((group) => ({ _id: group._id, position: group.position }));
