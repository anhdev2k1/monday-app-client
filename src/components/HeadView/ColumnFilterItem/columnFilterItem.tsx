...
const ColumnFilterItem = ({ columnValue}: IColumnFilterItemProps) => {
   ...
   return (
      <>
         <div className={`menu__column-item ${activeFilterItem.includes(columnValue._id) ? 'active' : ''}`} key={columnValue._id} onClick={() => handleFilter(columnValue._id)}>
           <div className="menu__column-item-icon" style={{backgroundColor: columnValue.belongType.color}}>
